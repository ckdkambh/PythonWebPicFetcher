...
class ImgDownloader():
    ...
    def run(self):
        ...
        dbase = shelve.open("xrmn5_recorder")
        albumList = set()
        ...
        for i in range(self.start, self.end + 1):
            ...
            if i == 1:
                currentList = [x for x in self.get_album_list(baseUrl) if x['url'] not in albumList]
            else:
                currentList = [x for x in self.get_album_list(baseUrl + 'index%d.html' % i) if x['url'] not in albumList]
